...
import com.colinalworth.xmlview.client.validator.XmlValidator.SchemaPath;
import com.colinalworth.xmlview.client.validator.XmlValidator.SchemaURL;
import com.colinalworth.xmlview.client.validator.impl.AbstractXmlValidatorImpl;
...
public class XmlValidatorGenerator extends Generator {
	...
	public String generate(TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException {
		...
		JClassType xmlValidator = oracle.findType(Name.getSourceNameForClass(XmlValidator.class)).isInterface();
		if (!xmlValidator.isAssignableFrom(toGenerate)) {
		...
		factory.setSuperclass(Name.getSourceNameForClass(AbstractXmlValidatorImpl.class));
	...
	private static class ValidatorCreator {
		...
		private Map<XSElementDecl, Set<XSElementDecl>> elementChildren;
		private Map<XSElementDecl, Set<XSAttributeDecl>> elementAttributes;
		/**
		 * 
		 */
		public ValidatorCreator(String simpleSourceName, Sources sources, GeneratorContext context, 
			...
			this.elementChildren = new HashMap<XSElementDecl, Set<XSElementDecl>>();
			this.elementAttributes = new HashMap<XSElementDecl, Set<XSAttributeDecl>>();
			...
			try {
				...
				TreeLogger parsingLogger = logger.branch(Type.INFO, "Reading schema");
				while (!elements.isEmpty()) {
					XSElementDecl elt = elements.removeFirst();
					TreeLogger l = parsingLogger.branch(Type.INFO, elt.getName());
					...
					if (elt.getType().isSimpleType()) {
						l.log(Type.INFO, elt.getType().getName());
					} else {
						XSComplexType complex = elt.getType().asComplexType();
						...
						if (content != null && content.getTerm().isModelGroup()) {
							for (XSParticle p : content.getTerm().asModelGroup().getChildren()){
		...
		public void generateMethods() throws UnableToCompleteException {
			TreeLogger l = logger.branch(Type.INFO, "Creating public validation methods");
			//JClassType xmlValidator = o.findType(Name.getSourceNameForClass(XmlValidator.class)).isInterface();

			//			protected boolean canElementHaveChildWithName(Element parentNode, String eltName);
			sw.println("protected native boolean canElementHaveChildWithName(Element parentNode, String eltName) /*-{");
			sw.indent();
			sw.println("var elementsChildren = %1$s[parentNode.@com.google.gwt.xml.client.Element::getNodeName()()];", generateElementChildMap());
			sw.println("return true == (elementsChildren && elementsChildren[eltName]);");//if no array exists, something is wrong with parent
			sw.outdent();
			sw.println("}-*/;");
			//			protected boolean isElementAllowedInCurrentPosition(Element parentNode, Element child);
			sw.println("protected boolean isElementAllowedInCurrentPosition(Element parentNode, Element child) {");
			sw.indent();
			sw.println("return true;");
			sw.outdent();
			sw.println("}");
			//
			//			protected boolean canElementHaveAttributeWithName(Element parentNode, String name);
			sw.println("protected native boolean canElementHaveAttributeWithName(Element parentNode, String name) /*-{");
			sw.indent();
			sw.println("var elementsAttrs = %1$s[parentNode.@com.google.gwt.xml.client.Element::getNodeName()()];", generateElementAttributeMap());
			sw.println("return true == (elementsAttrs && elementsAttrs[name]);");//if no array exists, something is wrong with parent...
			sw.outdent();
			sw.println("}-*/;");
			//			protected boolean doesElementHaveAnotherAttributeWithSameName(Element parentNode, String name);
			//			sw.println("protected boolean doesElementHaveAnotherAttributeWithSameName(Element parentNode, String name) {");
			//			sw.indent();
			//			sw.println("return true");
			//			sw.outdent();
			//			sw.println("}");
			//
			//			protected boolean isValueValidInAttributeInElement(String value, String name, Element parentNode);
			sw.println("protected boolean isValueValidInAttributeInElement(String value, String name, Element parentNode) {");
			sw.indent();
			sw.println("return true;");
			sw.outdent();
			sw.println("}");
			//
			//
			//			protected boolean doesDataMatchRestriction(Element parentNode, String data);
			sw.println("protected boolean doesDataMatchRestriction(Element parentNode, String data) {");
			sw.indent();
			sw.println("return true;");
			sw.outdent();
			sw.println("}");
			//			protected boolean canElementHaveCData(Element elt);
			sw.println("protected native boolean canElementHaveCData(Element elt) /*-{");
			sw.indent();
			sw.println("return true == (%1$s[elt.@com.google.gwt.xml.client.Element::getNodeName()()]);", generateElementsWithNonElementContents());
			sw.outdent();
			sw.println("}-*/;");

		}
		private String generateElementChildMap() {
			//first layer is parent, second is child:true (or not)
			JSONObject outer = new JSONObject();

			try {
				for (XSElementDecl parent : this.elementChildren.keySet()) {
					JSONObject inner = new JSONObject();
					for (XSElementDecl child : this.elementChildren.get(parent)) {
						inner.put(child.getName(), true);
					}
					outer.put(parent.getName(), inner);
				}
		...
		private String generateElementAttributeMap() {
			//first layout is element, second is attr:true (or not)
			JSONObject outer = new JSONObject();

			try {
				for (XSElementDecl parent : this.elementAttributes.keySet()) {
					JSONObject inner = new JSONObject();
		...
		private String generateElementsWithNonElementContents() {
			...
			try {
				for (XSElementDecl elt : this.elementAttributes.keySet()) {
					if (elt.getType().isSimpleType()) {
						map.put(elt.getName(), true);
					}
				}
			} catch (JSONException e) {
				e.printStackTrace();
